...
#load peak files
peak_files <- list(C5_C9_common_ac = paste0(dir, "c5_c9_ac_overlap_peaks.bed"),
                   WT_unique_ac = paste0(dir, "WT_ac_unique_peaks.bed"),
                   C5_unique_ac = paste0(dir, "C5_ac_unique_peaks.bed"),
                   C9_unique_ac = paste0(dir, "C9_ac_unique_peaks.bed"), 
                   WT_C5_C9_overlap_ac = paste0(dir, "wt_c5_c9_ac_overlap_peaks.bed"))


#Annotate peaks for each condition and save files
annotated_peaks_WT_unique_ac <- annotate_peaks(peak_files, "WT_unique_ac") 
annotated_peaks_C5_unique_ac <- annotate_peaks(peak_files, "C5_unique_ac")
annotated_peaks_C9_unique_ac <- annotate_peaks(peak_files, "C9_unique_ac")
annotated_peaks_C5_C9_common_ac <- annotate_peaks(peak_files, "C5_C9_common_ac") 
annotated_peaks_WT_C5_C9_overlap_ac <- annotate_peaks(peak_files, "WT_C5_C9_overlap_ac")


create.dir("./CUT_and_RUN/1_2_Snakemake_run_merged_replicates/seacr/contrasts/annotated_peaks")

write.csv(annotated_peaks_WT_unique_ac, file = paste0(dir, "./annotated_peaks/WT_unique_ac.csv"), quote = FALSE, row.names = FALSE)
write.csv(annotated_peaks_C5_unique_ac, file = paste0(dir, "./annotated_peaks/C5_unique_ac.csv"), quote = FALSE, row.names = FALSE)
write.csv(annotated_peaks_C9_unique_ac, file = paste0(dir, "./annotated_peaks/C9_unique_ac.csv"), quote = FALSE, row.names = FALSE)
write.csv(annotated_peaks_C5_C9_common_ac, file = paste0(dir, "./annotated_peaks/C5_C9_common_ac.csv"), quote = FALSE, row.names = FALSE)
write.csv(annotated_peaks_WT_C5_C9_overlap_ac, file = paste0(dir, "./annotated_peaks/WT_C5_C9_overlap_ac.csv"), quote = FALSE, row.names = FALSE)
...
C5_unique_ac_summary <- as.data.frame(table(annotated_peaks_C5_unique_ac$annotation))
colnames(C5_unique_ac_summary) <- c("Annotation", "C5_unique_ac")
...
C5_C9_common_ac_summary <- as.data.frame(table(annotated_peaks_C5_C9_common_ac$annotation))
colnames(C5_C9_common_ac_summary) <- c("Annotation", "C5_C9_common_ac")

WT_C5_C9_overlap_ac_summary <- as.data.frame(table(annotated_peaks_WT_C5_C9_overlap_ac$annotation))
colnames(WT_C5_C9_overlap_ac_summary) <- c("Annotation", "WT_C5_C9_overlap_ac")



summary_annotation_ac <- left_join(WT_unique_ac_summary, C5_unique_ac_summary, by = "Annotation") %>%
  left_join(C9_unique_ac_summary, by = "Annotation") %>%
  left_join(C5_C9_common_ac_summary, by = "Annotation") %>%
  left_join(WT_C5_C9_overlap_ac_summary, by = "Annotation") %>%
...
summary_annotation_ac$Condition <- factor(summary_annotation_ac$Condition, 
                                           levels = c("WT_C5_C9_overlap_ac", "C5_C9_common_ac", "C9_unique_ac", "C5_unique_ac", "WT_unique_ac"))
...
ggsave(filename = "./CUT_and_RUN/plots/annotated_contrast_peaks_ac.pdf", plot = annotated_ac_peaks_plot, 
...
C5_unique_ac_summary_perc <- C5_unique_ac_summary %>%
  mutate(C5_unique_ac_percentage = C5_unique_ac/sum(C5_unique_ac_summary$C5_unique_ac)*100) %>%
  dplyr::select(-C5_unique_ac) 
...
C5_C9_common_ac_summary_perc <- C5_C9_common_ac_summary %>%
  mutate(C5_C9_common_ac_percentage = C5_C9_common_ac/sum(C5_C9_common_ac_summary$C5_C9_common_ac)*100) %>%
  dplyr::select(-C5_C9_common_ac) 

WT_C5_C9_overlap_ac_summary_perc <- WT_C5_C9_overlap_ac_summary %>%
  mutate(WT_C5_C9_overlap_ac_percentage = WT_C5_C9_overlap_ac/sum(WT_C5_C9_overlap_ac_summary$WT_C5_C9_overlap_ac)*100) %>%
  dplyr::select(-WT_C5_C9_overlap_ac) 

summary_annotation_perc <- left_join(WT_unique_ac_summary_perc, C5_unique_ac_summary_perc, by = "Annotation") %>%
  left_join(C9_unique_ac_summary_perc, by = "Annotation") %>%
  left_join(C5_C9_common_ac_summary_perc, by = "Annotation") %>%
  left_join(WT_C5_C9_overlap_ac_summary_perc, by = "Annotation") %>%
...
summary_annotation_perc$Condition <- factor(summary_annotation_perc$Condition,
                                            levels = c("WT_C5_C9_overlap_ac_percentage",
                                                       "C5_C9_common_ac_percentage",
                                                       "C9_unique_ac_percentage",
                                                       "C5_unique_ac_percentage",
...
ggsave(filename = "./CUT_and_RUN/plots/annotated_constrast_ac_peaks_percentage.pdf", 
