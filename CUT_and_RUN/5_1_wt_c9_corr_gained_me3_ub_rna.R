...
#Output from annotate_peaks.R script
annotated_peaks_c9_me3_ub_genes <- annotated_peaks_c9_me3_ub %>%
  dplyr::filter(SYMBOL != "NA")

expression_wt_vs_c9 <- read.csv("./RNA_Seq/results_files/normalised_expression_aml2_wt_vs_c9.csv") %>%
  dplyr::filter(geneID %in% annotated_peaks_c9_me3_ub_genes$SYMBOL)

corr_wt_vs_c9_wt_me3_ub_peaks <- annotated_peaks_c9_me3_ub_genes %>%
...
plot_regions_gaining_me3_ub_c9 <- corr_wt_vs_c9_wt_me3_ub_peaks %>%
  dplyr::filter(annotation!= "Distal Intergenic") %>%
    ...
    geom_point(aes(fill=Condition, group = SYMBOL), size=2, shape=21, position = position_dodge(0.6)) +
    ...
    labs(x = "Condition", y = "RNA expression\nlog2(tpm+1)", title = "Regions gaining me3&ub in C9") #+
  facet_wrap(~annotation, scales = "free", nrow = 2)
  
ggsave(plot_regions_gaining_me3_ub_c9, file = "./CUT_and_RUN/plots/regions_gaining_me3_ub_c9.pdf",
